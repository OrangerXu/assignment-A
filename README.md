...
| 软件 | 版本 |
|------|------|
| 操作系统 | Centos7 (VMware Workstation) |
| JDK | OpenJDK 1.8.0 |
...
### 3.4实验结果分析
#### 3.4.1 不同大小数据集的最佳策略
1. 对于1MB数据集，最佳策略为`Default`
2. 对于10MB数据集，最佳策略为`Default`
3. 对于30MB数据集，最佳策略为`AllMapDone`

#### 3.4.2分析

- **性能与数据规模的相关性分析**：
  - **1MB数据集**：`Default (0.05)`策略最优。由于数据量极小，Map阶段快速完成，适度的Reduce提前启动（5%）能够充分利用系统资源，实现最佳的流水线效果，而过早启动（0.0）会引入不必要的调度开销。即使1MB数据，过早启动Reduce仍会与Map竞争有限资源。
  
  - **10MB数据集**：`Default (0.05)`继续保持最优。在此规模下，Hadoop默认策略展现了最佳的平衡性，既避免了过早启动导致的资源竞争，又充分利用了阶段重叠的并行优势。单机资源瓶颈使小数据就达到集群中大数据的竞争程度。
  
  - **30MB数据集**：`AllMapDone (1.0)`成为最优策略。随着数据规模增大，Map阶段执行时间显著延长。此时，过早启动Reduce任务（如0.0、0.05、0.5）会导致严重的资源争用，特别是在单机伪分布式环境下，CPU和内存资源的竞争尤为激烈。等待所有Map完成后再启动Reduce，虽然牺牲了部分并行性，但避免了资源竞争带来的性能损耗。30MB就触发了严重的资源竞争。


### 3.5 结论
- **Reduce启动策略的选择与输入数据规模密切相关，不存在适用于所有场景的最优解**。需要根据具体的数据量和集群资源配置进行针对性调优。

- **`Default (0.05)`策略在中小规模数据集上表现稳健**：在1MB和10MB场景下均取得最优性能，证明了Hadoop默认参数设置的合理性，是通用场景下的安全选择。

- **`AllMapDone (1.0)`策略在较大规模单机环境中具有优势**：当数据规模增长到30MB时，避免资源竞争比追求阶段重叠更为重要，此时串行执行Map和Reduce阶段反而能获得更好的整体性能。

- **本研究通过实际实验验证了不同Reduce启动策略的性能特性**，为Hadoop作业在单机伪分布式环境下的调优提供了实践依据，强调了根据数据规模动态调整策略的重要性。


## 四、任务三（TeraSort 较小数据集）
...
## 六、分工说明
- **[徐晨]**：负责任务一 WordCount（较大数据集） 集群环境搭建、实验脚本编写与调试、实验执行、数据收集与分析、报告撰写。录制演示视频与剪辑。收集文档与材料，完成Github项目assignment-A搭建。
- **[郝以婷]**：负责任务二 WordCount（较小数据集） 集群环境搭建、实验脚本编写与调试、实验执行、数据收集与分析、报告撰写。录制演示视频。组织分工。
- **[宗宣君]**：负责任务三 TeraSort（较小数据集） 集群环境搭建、实验脚本编写与调试、实验执行、数据收集与分析、报告撰写。录制演示视频。完成PPT制作。
- **[马百腾]**：负责任务四 TeraSort（较大数据集） 集群环境搭建、实验脚本编写与调试、实验执行、数据收集与分析、报告撰写。录制演示视频。
